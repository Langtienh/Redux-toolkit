...
import {
  ...
  cancelEditPost,
  updatePost,
} from "@/redux/builder/blog.reducer";
...
export default function CreatePost() {
  ...
  const postEdit = useSelector(
    (state: RootState) => state.blogBuilder.editingPost
  );
  useEffect(() => {
  ...
  const onSubmit = (even: FormEvent<HTMLFormElement>) => {
    even.preventDefault();
    const data = { ...formData, id: generateRandomId() };
    if (postEdit) dispatch(updatePost(formData));
