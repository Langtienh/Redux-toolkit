...
export default function CreatePost() {
  ...
  const postEdit = useSelector(
    (state: RootState) => state.blogSlice.editingPost
  );
  useEffect(() => {
  ...
  const onSubmit = (even: FormEvent<HTMLFormElement>) => {
    even.preventDefault();
    if (postEdit) dispatch(updatePostSlice(formData));
    else dispatch(addPostSlice(formData));
    setFormData(initialState);
  };
  const handleCancelEdit = () => {
    dispatch(cancelEditPostSlice());
  };
  const handleBackup = () => {
    dispatch(backupPostSlice());
  };
  return (
